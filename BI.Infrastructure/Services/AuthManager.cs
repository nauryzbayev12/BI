...
namespace BI.Infrastructure.Services
{
    public class AuthManager : IAuthManager
    {
        ...
        {
            await _userManager.RemoveAuthenticationTokenAsync(_user, "BIApi", "RefreshToken");
            var newRefreshToken = await _userManager.GenerateUserTokenAsync(_user, "BIApi", "RefreshToken");
            var result = await _userManager.SetAuthenticationTokenAsync(_user, "BIApi", "RefreshToken", newRefreshToken);
        ...
        public async Task<TokenRequest> VerifyRefreshToken(TokenRequest request)
        {
            ...
            try
            {
                var isValid = await _userManager.VerifyUserTokenAsync(_user, "BIApi", "RefreshToken", request.RefreshToken);
                if (isValid)
                {
